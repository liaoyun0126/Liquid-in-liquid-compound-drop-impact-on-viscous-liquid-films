...
scalar f1[], f2[], f3[];
scalar * list = NULL;
double Ohd, Ohf, Ohs, hf, mu1, mu2, mu3, mu4;
...
int main(int a, char const *arguments[]){
  ...
  mu1 = Ohd; mu2 = Ohf; mu3 = Ohf; mu4 = 1e-5;
...
//  double Uc = 0., wt = 0., Deltax, Deltay;
  ...
  foreach(){
    ...
    double mu = clamp(f1[],0.,1.)*mu1 + clamp(f2[],0.,1.)*mu2 + clamp(f3[],0.,1.)*mu3 + clamp(1.-f1[]-f2[]-f3[],0.,1.)*mu4;

...
def gettingfield(filename):
    ...
    temp1 = stderr.decode("utf-8")
    temp2 = temp1.split("\n")
    Rtemp, Ztemp, f1temp, f2temp, f3temp, D2temp, Omegatemp, Utemp, Vtemp, veltemp = [] , [], [], [], [], [], [], [], [], []
    ...
    for n1 in range(len(temp2)):
        ...
        if temp3 == ['']:
            ...
            f3temp.append(float(temp3[4]))
            D2temp.append(float(temp3[5]))
            Omegatemp.append(float(temp3[6]))
            Utemp.append(float(temp3[7]))
            Vtemp.append(float(temp3[8]))
            veltemp.append(float(temp3[9]))

    R = np.asarray(Rtemp)
    Z = np.asarray(Ztemp)
    f1 = np.asarray(f1temp)
    f2 = np.asarray(f2temp)
    f3 = np.asarray(f3temp)
    ...
    return R, Z, f1, f2, f3, D2, Omega, U, V, vel
# ------------------------------------------------------------------------------
...
folder = 'VideoBas2'  # output folder
if not os.path.isdir(folder):
    os.makedirs(folder)

rmin, rmax, zmin, zmax = [-4.0, 4.0, -hf, 4.0-hf]
...
for ti in range(nGFS):
    ...
    name = "%s/%4.4d.svg" %(folder, int(t*1e3))
    if not os.path.exists(place):
        ...
        if os.path.exists(name):
            ...
            if (len(facets1)):
                ...
                facets2 = gettingFacets(place, 3)
                R, Z, f1, f2, f3, D2, Omega, U, V, vel = gettingfield(place)
                ...
                ax.plot([0, 0], [zmin, zmax],'-.',color='black',linewidth=lw/2)
                ...
                ## omega
                cntrl1 = ax.imshow(D2, interpolation='bilinear', cmap="gray_r", origin='lower', extent=[0, -rmax, zmin, zmax], vmax = 1, vmin = -3)

                ## V
                maxs = speed.max()
                if maxs > 0.:
                    MaxVel = np.sqrt(We)
                    cntrl2 = ax.imshow(speed, interpolation='bilinear', cmap="YlGn", origin='lower', extent=[0, rmax, zmin, zmax], vmax = 2, vmin = 0.)

                line_segments1 = LineCollection(facets1, linewidths=4, colors='blue', linestyle='solid')
                ax.add_collection(line_segments1)
                line_segments2 = LineCollection(facets2, linewidths=4, colors='blue', linestyle='solid')
                ax.add_collection(line_segments2)
                line_segments1 = LineCollection(facets3, linewidths=4, colors='blue', linestyle='solid')
